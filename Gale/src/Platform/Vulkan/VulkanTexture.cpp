...
namespace Gale {
	...
	VulkanTexture::VulkanTexture(Ref<VulkanDevice> device, const std::string& texFilepath):
		m_Device(device)
	{
	...
	void VulkanTexture::createTexture(const std::string& texFilepath)
	{
		...
		if (!pixels)
			GL_ERROR("failed to load texture image!");
		...
		void* data;
		vkMapMemory(m_Device->device(), textureBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, pixels, static_cast<size_t>(bufferSize));
		vkUnmapMemory(m_Device->device(), textureBufferMemory);
	...
	void VulkanTexture::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		...
		else if (oldLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL && newLayout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
		{
			...
			GL_ERROR("unsupported layout transition!");
